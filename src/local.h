...
char to_lower(char c) { return c - 'A' + 'a'; }
...
/// Set the mask such that the k-mer corresponding to the given interval is
/// masked as ghost.
void delete_k_mer(sdsl::bit_vector &mask, size_t start, size_t count,
                  size_t start_rc, size_t count_rc, assignable_function_t f) {
  bool zeroing = false;
  for (size_t i = 0; i < count; ++i) {
    if (f(i, count + count_rc) == 0)
      zeroing = true;
    mask[start + i] = !zeroing;
  }
  for (size_t i = 0; i < count_rc; ++i) {
    if (f(i + count, count + count_rc) == 0)
      zeroing = true;
...
bool query_f_and_delete(fm_index_t &fm_index, sdsl::bit_vector &mask,
...
/// Return the extension - that is the d chars extending the simplitig - and the
/// extending kMer.
std::string extension(fm_index_t &fm_index, sdsl::bit_vector &mask,
                      std::string &base, assignable_function_t f, int d,
                      bool left) {
  ...
  for (int ext = 0; ext < (1 << (d << 1)); ++ext) {
    ...
    if (query_f_and_delete(fm_index, mask, f, next)) {
      return decoded_ext;
    }
  }
  return "";
...
masked_superstring_t merge_superstrings(masked_superstring_t a,
                                        masked_superstring_t b) {
...
masked_superstring_t next_generalized_simplitig(fm_index_t &fm_index,
                                                sdsl::bit_vector &mask,
                                                assignable_function_t f,
                                                int d_max, std::string &begin) {
  ...
  while (d_l <= d_max || d_r <= d_max) {
    if (d_r <= d_l) {
      ...
      auto ext = extension(fm_index, mask, base, f, d_r, false);
      if (ext.empty()) {
      ...
      auto ext = extension(fm_index, mask, base, f, d_l, true);
      if (ext.empty()) {
  ...
  return merge_superstrings({mask_front, simplitig_front},
                            {mask_back, simplitig_back});
}

/// Find the next k-mer to start a new generalized simplitig.
std::string next_k_mer(fm_index_t &fm_index, sdsl::bit_vector &mask,
                       klcp_t &klcp, assignable_function_t f, size_t &start,
                       int k) {
  while (start < fm_index.size()) {
    ...
    start = end;
    if (query_f_and_delete(fm_index, mask, f, k_mer)) {
      return k_mer;
    }
  }
  // No more k-mers.
...
/// Construct a masked superstring from the FM-index using the local greedy.
masked_superstring_t local(fm_index_t &fm_index, sdsl::bit_vector &mask,
                           klcp_t &klcp, assignable_function_t f, int k,
                           int d_max) {
  size_t start = 0;
  masked_superstring_t ret;
  while (start < fm_index.size()) {
    auto k_mer = next_k_mer(fm_index, mask, klcp, f, start, k);
    if (k_mer.empty())
      break;
    ret = merge_superstrings(
        ret, next_generalized_simplitig(fm_index, mask, f, d_max, k_mer));
  }
  return ret;
}